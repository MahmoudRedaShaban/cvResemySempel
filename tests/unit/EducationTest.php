<?php
use Laravel\Lumen\Testing\DatabaseMigrations;
use Laravel\Lumen\Testing\DatabaseTransactions;
use PHPUnit\Util\Json;

class EducationTest extends TestCase
{
    public $authuser = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..dYACEOFxUz5oYvD8kQfmJqR5NjuiXXm7JBQkYpvjxIJfPjXO3pigsJUaCXKcV_p8dDngYjLYEhDHNHEVi8DU3QkcIDlV_LjrDWxzs7lmHDbs2YCanuHOYoDZxzQd09WALK1yu3CGNkn6G5BQHfvRuCB_VCahYB2e5ZMfh1tnyJZcGkqNw2SsPuVkWTjQbqrzyfnhirUG9-hL0ekdsLLyGluTkJIs43SLSOisQkP24rtba5IyhRd4R7ziZoFFj3Nj8P2YgaKpWb0wLuD0kAqtEojWw9LMOx-9wE-0QWoO9YdEkQJgY7ukViccnwjBahFvaQlpNzyULw4ovvz0CCSsW3LXHhnDVe-GSpOucQhU755-YEF5eELKNEBRpwagw4rpLcprwJvkk7qtjJk_HbMv0iPDGiXvqSPQF8XxYReeiBwMmWvFwN-1kp3aAprHOGk-pX5-TwDQ8jbLJwRGnGgrdMXNdqyJ67YuQfNiWixNoR8gKS1CfusKT42WWzt0wWhDury80b742hCFFNxKpMlZwaL0a1-GQxvq8FPY-2J2VeZMZpwCWSe0Sd5rrvrwMOATfIo7iUPim5O6wBU_XMtLuKM3i8fLkAbkv1lX9QCwu47bqfJFtj3xq9h9iVCNweT0Q1xTDKI8y34534BdVKD-gKZWdedDbXvTBccpLBCg3OI";
    public $id_insert;
    /**
     * Test For Insert Education
     *
     * @test
     */
    public function insert_new_Education()
    {

        $this->post('/api/education', [
            "school_name"=>"azhar", "school_location"=>"cairo", "degree"=>"Good", "field_of_study"=>"business",
            "graduation_start_date"=>"2016-04-24T02:45:55.000000Z", "graduation_end_date"=>"2019-04-24T02:45:55.000000Z"
        ], [
            "Content-Type" => "application/json",
            "Authorization" => $this->authuser
        ]);
        $this->id_insert = $this->response->getContent();
        $this->assertJson($this->id_insert);
    }
    /**
     * Test For Get Singl  Education
     *
     */
    public function testGetSinglByID()
    {
        $this->get('/api/education/1', [
            "Content-Type" => "application/json",
            "Authorization" => $this->authuser
        ]);
        $this->response->assertOk();
    }


    /**
     * Test For Get All  Education
     *
     */
    public function testapi_for_get_all()
    {
        $req = $this->get('/api/education', [
            "Content-Type" => "application/json",
            "Authorization" => $this->authuser
        ]);

        $this->assertResponseStatus(200);
    }

    public function test_update_by_id()
    {
        $this->put('/api/education/1', [
            "school_name"=>"azhara", "school_locationa"=>"caira", "degree"=>"Gooda", "field_of_astudy"=>"business",
            "graduation_start_date"=>"2016-04-24T02:45:55.000000Z", "graduation_end_date"=>"2019-04-24T02:45:55.000000Z"
        ], [
            "Content-Type" => "application/json",
            "Authorization" => $this->authuser
        ]);
        // $this->assertResponseStatus(201);
        $this->assertJson($this->response->getContent());
    }
    /**
     * Test For Delete Singl By Id From   Education
     *
     */
    public function testDeletedDetaildsUser()
    {
        $this->delete('/api/education/1',[],[
            "Content-Type" => "application/json",
            "Authorization" => $this->authuser
        ]);
        $this->assertResponseStatus(200);
    }
}
